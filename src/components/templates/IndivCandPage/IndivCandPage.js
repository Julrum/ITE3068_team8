...
import { CandImage, Cell, CellList, PageTemplate, Profile, Profiles, Promise, Title } from './IndivCandPage.style'
...
const IndivCandPage = ({num, profileDatas, promiseDatas}) => {
    ...
    return <PageTemplate>
        <Profiles>
            <CandImage background={require(`../../../assets/image/${profileDatas.image}.jpeg`).default}></CandImage>


            <Profile>
                <Title>{profileDatas.name}</Title>  
                {profileDatas.repProfile.map((item) => (
            ...
            <CellList>
                {
                    promiseDatas.area.map((item, index) => (
                        <Cell onClick={() => openPromiseModal(index)}>
                            <PromiseCell area={item.title} data={item.rep_promise}></PromiseCell>
                        </Cell>
                    ))
                }
                {
                    promiseDatas.area.map((item, index) => (
                        <Cell>
                            <PromiseCell2 open={() => openPromiseModal(index)} area={item.title} data={item.rep_promise}></PromiseCell2>
                        </Cell>
                    ))
                }
            </CellList>
